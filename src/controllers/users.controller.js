...
export const createUser = async (req, res, next) => {
    try {
        ...
        if (usu_cpf && !isValidCPF(usu_cpf)) return res.status(400).json({ status: 'error', message: 'CPF inválido.' });
        if (usu_email && !isValidEmail(usu_email)) return res.status(400).json({ status: 'error', message: 'Formato de e-mail inválido.' });
        ...
        if (usu_senha && !isValidPassword(usu_senha)) {
            return res.status(400).json({
                status: 'error',
                message: 'A senha deve ter no mínimo 12 caracteres, maiúscula, minúscula, número e especial.'
            });
        ...
        const passwordHash = usu_senha ? await hashPassword(usu_senha) : null;
    ...
    `;
        ...
        // const values = [
        //     usu_nome, usu_cpf, usu_data_nasc || null, usu_sexo ?? null, usu_telefone || null, usu_email, usu_observ || null, usu_acesso ?? false,
        //     passwordHash,
        //     usu_situacao ?? true
        // ];
        ...
        const values = [
            usu_nome,
            usu_cpf || null, // Se vazio, vira null no banco
            usu_data_nasc || null,
            usu_sexo ?? null,
            usu_telefone || null,
            usu_email || null, // Se vazio, vira null no banco
            usu_observ || null,
            usu_acesso ?? false,
            passwordHash, // Pode ser null agora
            usu_situacao ?? true
        ];
